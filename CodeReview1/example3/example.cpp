...
private:
  ...
  const float calcVolume(const int &a) { return pow(a, 3) * sqrt(2) / 12; }

  const float calcArea(const int &a) { return pow(a, 2) * sqrt(3); }

  const float calcRU(const int &a) { return a * sqrt(6) / 4; }

  const float calcRI(const int &a) { return a * sqrt(6) / 12; }
